...
const ReSendVerify = () => {
    ...
    const [data, setData] = useState({
       ...
       const handleSumit = async (e) => {
        ...
        try {
          ...
          const res = await axios.post("/auth/resendemail", data);
          setMsg(true);
          setError(null);
        } catch (err) {
          console.log(err);
    ...
    return (
        <Container>
      <Wrapper>
        <Title>Gửi Lại Mã Xác Thực</Title>
        <Form>
          <Input placeholder="username" required onChange={e => setData({...data, username: e.target.value})} value= {data.username}/>
          <Input type="email" placeholder="email" required onChange={e => setData({...data, email: e.target.value})} value= {data.email}/>
          <Button onClick={handleSumit}>RESEND</Button>
          <Link href="/login">Go to Login</Link>
          <Link href="/register">CREATE A NEW ACCOUNT</Link>
          {msg==true && <SuccessBox>
            <AlertCloseButton onclick="this.parentElement.style.display='none';">&times;</AlertCloseButton>
            <strong>SUCCESS!</strong>  Mã xác nhận đã được gửi lại vào mail của bạn.
          </SuccessBox>}
          {error!=null && <Alert>
            <AlertCloseButton onclick="this.parentElement.style.display='none';">&times;</AlertCloseButton>
